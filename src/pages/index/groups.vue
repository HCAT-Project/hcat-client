...
<template>
  <!-- Chat -->
  <div flex w-full>
    <div grid="~ cols-12" w="9/12" rounded-2xl bg-back-gray of-hidden>
      <div col-span-4 flex="~ col" p="y5 x5" gap-5>
        ...
        <!-- Chat card -->
        <div flex="~ col">
          <GroupChatCard v-for="item, key in store.groupList" :key="item.group_name" :group_id="key" :selected="selectedGroup === key" :name="item.group_name" :new-message-number="99" @click="selectGroup(key as string)" />
        </div>
      </div>
      <!-- Chat window -->
          ...
          <div flex="~ col">
            <div flex="~ col" gap-1>
              <p text="lg start" font-bold>
                {{ store.groupList[selectedGroup]?.group_name ?? '群组' }}
              </p>
